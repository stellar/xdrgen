...
pub const XDR_FILES_SHA256: [(&str, &str); 1] = [(
    "spec/fixtures/generator/struct.x",
    "c6911a83390e3b499c078fd0c579132eacce88a4a0538d3b8b5e57747a58db4a",
)];
...
impl TypeVariant {
    pub const VARIANTS: [TypeVariant; 2] = [TypeVariant::Int64, TypeVariant::MyStruct];
    pub const VARIANTS_STR: [&'static str; 2] = ["Int64", "MyStruct"];
...
impl Type {
    ...
    pub fn read_xdr<R: Read>(v: TypeVariant, r: &mut Limited<R>) -> Result<Self> {
        match v {
            ...
            TypeVariant::MyStruct => {
                r.with_limited_depth(|r| Ok(Self::MyStruct(Box::new(MyStruct::read_xdr(r)?))))
            }
        }
    }
    ...
    pub fn read_xdr_iter<R: Read>(
        ...
        match v {
            ...
            TypeVariant::MyStruct => Box::new(
                ReadXdrIter::<_, MyStruct>::new(&mut r.inner, r.limits.clone())
                    .map(|r| r.map(|t| Self::MyStruct(Box::new(t)))),
            ),
        }
    }
    ...
    pub fn read_xdr_framed_iter<R: Read>(
        ...
        match v {
            ...
            TypeVariant::MyStruct => Box::new(
                ReadXdrIter::<_, Frame<MyStruct>>::new(&mut r.inner, r.limits.clone())
                    .map(|r| r.map(|t| Self::MyStruct(Box::new(t.0)))),
            ),
        }
    }
    ...
    pub fn read_xdr_base64_iter<R: Read>(
        ...
        match v {
            ...
            TypeVariant::MyStruct => Box::new(
                ReadXdrIter::<_, MyStruct>::new(dec, r.limits.clone())
                    .map(|r| r.map(|t| Self::MyStruct(Box::new(t)))),
            ),
        }
    }

...
// MustDataC retrieves the DataC value from the union,
// panicing if the value is not set.
func (u ContainerStructBody) MustDataC() uint64 {
  val, ok := u.GetDataC()

  if !ok {
    panic("arm DataC is not set")
  }

  return val
}

// GetDataC retrieves the DataC value from the union,
// returning ok if the union's switch indicated the value is valid.
func (u ContainerStructBody) GetDataC() (result uint64, ok bool) {
  armName, _ := u.ArmForSwitch(int32(u.Type))
...
return nil
    case UnionTypeUnionC:
        if   _, err = e.EncodeUhyper(uint64((*u.DataC))); err != nil {
    return err
  }
return nil
    case UnionTypeUnionD:
...
switch UnionType(u.Type) {
  ...
  return n, nil
    case UnionTypeUnionC:
        u.DataC = new(uint64)
  (*u.DataC), nTmp, err = d.DecodeUhyper()
  n += nTmp
  if err != nil {
    return n, fmt.Errorf("decoding Unsigned hyper: %w", err)
  }
